...
export type RobotState = {
  ...
  backtracks: number;
  status: string;
  mode: "idle" | "dry" | "actual" | "done";
  route: Cell[];
  actualCursor: number;
  led: boolean;
  firstEndReached: boolean;
  drySteps: number;
  discoveredCheckpoints: Set<string>;
  junctions: JunctionMemory[];
...
export function edge(a: Cell, b: Cell): string {
...
export function createRobot(start: Cell): RobotState {
  return {
    pos: { ...start },
    heading: "E",
    known: { cells: new Set([key(start)]), edges: new Set() },
    trace: [{ ...start }],
    backtracks: 0,
    status: "Ready",
    mode: "idle",
    ...
    drySteps: 0,
    discoveredCheckpoints: new Set(),
    junctions: [],
    edgeTraversals: new Map(),
  };
}

export function sense(m: Maze, r: RobotState) {
...
function local(m: Maze, r: RobotState) {
  ...
  return senseLocal(m, r.pos).sort(
    (a, b) => rank.get(a.dir)! - rank.get(b.dir)!,
  );
}

function visitEdge(r: RobotState, a: Cell, b: Cell) {
  const k = edge(a, b);
  r.edgeTraversals.set(
    k,
    Math.min(2, (r.edgeTraversals.get(k) ?? 0) + 1) as 0 | 1 | 2,
  );
}

function discoverCheckpoint(m: Maze, r: RobotState) {
  const k = key(r.pos);
  if (m.checkpoints.has(k)) r.discoveredCheckpoints.add(k);
}

/**
 * Trémaux-style exploration. The robot only uses local exits and its own
 * traversal memory. Dry Run ends when the simulated END sensor activates.
 */
export function buildDryTrace(m: Maze, start: Cell): DryRunResult {
  const r = createRobot(start);
  r.mode = "dry";
  r.status = "Exploring from local sensors";
  discoverAt(m, r);
  discoverCheckpoint(m, r);

  const trace: Cell[] = [{ ...start }];
  ...
  while (stack.length) {
    ...
    if (exits.length > 2) {
      ...
      if (existing) {
        existing.exits = exits.map((x) => x.dir);
      } else {
        r.junctions.push({
          position: { ...current },
          exits: exits.map((x) => x.dir),
          visited: true,
        });
      }
    }
    ...
    const next = exits
      ...
      .sort((a, b) => a.traversals - b.traversals)[0];

    if (!next) {
      stack.pop();
      if (!stack.length) break;
      const parent = stack[stack.length - 1];
      visitEdge(r, current, parent);
      trace.push({ ...parent });
      backtracks++;
      r.pos = { ...parent };
      r.heading = dir(current, parent);
      discoverAt(m, r);
      discoverCheckpoint(m, r);
      continue;
    }
  ...
  return {
    trace,
    known: {
      cells: new Set(r.known.cells),
      edges: new Set(r.known.edges),
    },
    endReached: firstEndIndex >= 0,
    endCell,
    backtracks,
    firstEndIndex,
    discoveredCheckpoints: new Set(r.discoveredCheckpoints),
    junctions: r.junctions.map((j) => ({
      ...j,
      position: { ...j.position },
      exits: [...j.exits],
    })),
...
function parse(v: string): Cell {
  const [x, y] = v.split(",").map(Number);
  return { x, y };
}

/** Shortest route through the graph the robot has actually learned. */
export function shortestKnownRoute(
  g: KnownGraph,
  start: Cell,
  target: Cell,
): Cell[] {
...
export function simplifyTurns(route: Cell[]): string[] {
  ...
  for (let i = 1; i < route.length - 1; i++) {
    const a = dir(route[i - 1], route[i]);
    const b = dir(route[i], route[i + 1]);
    out.push(
      b === a
        ? "S"
        : b === opposite(a)
...
export function followStoredRoute(
  r: RobotState,
  route: Cell[],
  cursor: number,
): RobotState {
  ...
  const next = {
    ...
    actualCursor: i,
    status: i >= route.length - 1 ? "END reached" : "Replaying stored route",
    led: i >= route.length - 1,
  };
