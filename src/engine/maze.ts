...
/** The organiser/environment model. referenceShortest is observer-only benchmark data captured at dry-run start. */
export type Maze = {
  size: number;
  roads: Set<string>;
  start: Cell;
...
export function makeMaze(size = 23): Maze {
  return { size, roads: new Set<string>(), start: { x: 2, y: 20 }, end: { x: 20, y: 2 }, referenceShortest: [] };
}
export function cloneMaze(m: Maze): Maze {
  return { ...m, roads: new Set(m.roads), start: { ...m.start }, end: { ...m.end }, referenceShortest: m.referenceShortest.map(c => ({ ...c })) };
}
